...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, mask=None, layer_past=None):
        ...
        if mask is not None:
            ...
            att = att.masked_fill(mask[:,:,:T,:T] == 0, -1e10)
...
class Block(nn.Module):
    ...
    def forward(self, x, masks=None, layer_past=None, return_present=False):
        # TODO: check that training still works
        if return_present: assert not self.training
        # layer past: tuple of length two with B, nh, T, hs
        attn, present = self.attn(self.ln1(x), masks, layer_past=layer_past)
...
class GPT(nn.Module):
    ...
    def __init__(self, args, vocab_size, block_size, n_layer=12, n_head=8, n_embd=256,
                 embd_pdrop=0., resid_pdrop=0., attn_pdrop=0., n_unmasked=0, vtokens_pos=False,head_output_dim=256):
        super().__init__()
        config = GPTConfig(vocab_size=vocab_size, block_size=block_size,
                           embd_pdrop=embd_pdrop, resid_pdrop=resid_pdrop, attn_pdrop=attn_pdrop,
                           n_layer=n_layer, n_head=n_head, n_embd=n_embd,
                           n_unmasked=n_unmasked,head_output_dim=head_output_dim)
        ...
        self.head = nn.Linear(config.n_embd, config.head_output_dim, bias=False)
    ...
    def forward(self, idx=None, embeddings=None, masks=None,targets=None, cbox=None, tbox=None):
        ...
        x = self.drop(token_embeddings) #not using position embeddings
