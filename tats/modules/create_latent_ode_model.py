...
# from .modules.flow_transformer import FlowTransformerEncoder
#####################################################################################################

def create_LatentODE_model(args, input_dim, z0_prior, obsrv_std, device,decoder = None ,
	classif_per_tp = False, n_labels = 1):

	dim = args.latents
	if args.poisson:
		lambda_net = create_net(dim, input_dim, 
			n_layers = 1, n_units = args.units, nonlinear = nn.Tanh)

		# ODE function produces the gradient for latent state and for poisson rate
		ode_func_net = create_net(dim * 2, args.latents * 2, 
		...
		dim = args.latents 
		ode_func_net = create_net(dim, args.latents, 
			n_layers = args.gen_layers, n_units = args.units, nonlinear = nn.Tanh)
	...
	elif args.z0_encoder == "transformer":
		encoder_z0 = FlowTransformerEncoder(args = args.transformer_args, max_seq_len = 500,dim_z0 =z0_dim,
			input_feature_dim= int(input_dim), device=device ).to(device)
