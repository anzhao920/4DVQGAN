...
class VAE_Baseline(nn.Module):
	...
	def compute_all_losses(self, batch_dict, n_traj_samples = 1, kl_coef = 1.):
		# Condition on subsampled points
		# Make predictions for all the points
		all_pred_y, info = self.get_reconstruction(batch_dict["tp_to_predict"], 
			batch_dict["observed_data"], batch_dict["observed_tp"], 
			mask = batch_dict["observed_mask"], n_traj_samples = n_traj_samples)

		#print("get_reconstruction done -- computing likelihood")
		fp_mu, fp_std, fp_enc = info["first_point"]
		fp_std = fp_std.abs()+1e-7
		...
		# 	mask = batch_dict["mask_predicted_data"])
		if len(batch_dict["observed_tp"])==1:
			time = int(batch_dict["observed_tp"][0])
			pred_y = all_pred_y[:,:,time:time+1,:]
		else:
			pred_y = all_pred_y[:,:,batch_dict["observed_tp"].long(),:]
		# loss_mse = nn.MSELoss()
		# mse = self.get_mse(
		# 	batch_dict["data_to_predict"], pred_y,
		# 	mask = batch_dict["mask_predicted_data"])
		# mse = loss_mse(batch_dict["data_to_predict"], pred_y)
		# rec_likelihood = -mse
