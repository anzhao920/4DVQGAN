...
class VidODE(nn.Module):
    ...
    def build_model(self):
        ...
        init_dim = int(self.args.resolution/self.args.scale)
        ...
        input_size=(self.args.embedding_dim*2,self.args.embedding_dim*2)
        # ode_dim = base_dim
        ode_dim = self.args.embedding_dim
        ...
        self.encoder_z0 = Encoder_z0_ODE_ConvGRU(input_size=input_size,
                                                 ...
                                                 run_backwards=self.args.run_backwards).to(self.device)
        ...
        ##### Conv Decoder
        self.decoder = Decoder(input_dim=base_dim * 2, output_dim=self.input_dim + 3, n_ups=self.args.n_downs).to(self.device)
    ...
    def get_reconstruction(self, time_steps_to_predict, truth, truth_time_steps, mask=None, out_mask=None):
        ...
        resize = 2 ** self.args.n_downs
        b, t, c, h, w, d = truth.shape
        pred_t_len = len(time_steps_to_predict)
        
        # ##### Skip connection forwarding
        # skip_image = truth[:, -1, ...] if self.args.extrap else truth[:, 0, ...]
        # skip_conn_embed = self.encoder(skip_image).view(b, -1, h // resize, w // resize)
        
        ##### Conv encoding
        # e_truth = self.encoder(truth.view(b * t, c, h, w, d)).view(b, t, -1, h // resize, w // resize, d//resize)
        e_truth = truth
    ...
    def compute_all_losses(self, batch_dict):
        ...
        sol_y, sol_y_all = self.get_reconstruction(
